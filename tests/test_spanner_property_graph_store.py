...
pytestmark = pytest.mark.skipif(
    (
        not spanner_instance_id
        or not spanner_database_id
        or not spanner_graph_name
    ),
    reason=(
...
def test_filter_nodes_by_property(
  ...
  e3 = EntityNode(
      label="PERSON", name="Charlie", properties={"country": "France"}
  )
  for graph_store in [
      property_graph_store_static,
      property_graph_store_dynamic,
...
def test_vector_query(
  ...
  for graph_store in [
    ...
    entity1 = EntityNode(
        label="PERSON", name="Alice", embedding=[0.1, 0.2, 0.3]
    )
    ...
    query = VectorStoreQuery(
        query_embedding=[0.1, 0.2, 0.31], similarity_top_k=2
    )
...
def test_evolve_schema_add_new_property_to_existing_node_type_static(
  ...
  for node_type_info in schema_before.get(
      "Node properties per node type", {}
  ).get(f"{graph_store.schema.graph_name}_FRUIT", []):
  ...
  assert not any(
      p.get("property name") == "prop_ripeness" for p in node_props_before
  )
  ...
  assert any(
      p.get("property name") == "prop_color" for p in node_props_after_list
  )
  assert any(
      p.get("property name") == "prop_ripeness" for p in node_props_after_list
  )
...
def test_operations_after_cleanup_upsert_recreates_graph(
  ...
  assert (
      node_table_key in schema_after_recreation["Node properties per node type"]
  )
...
def test_schema_evolve_add_new_property_to_existing_edge_type_static(
  ...
  edge_props_before = schema_before["Edge properties per edge type"][
      edge_table_key
  ]
  assert any(
      p.get("property name") == "prop_distance" for p in edge_props_before
  )
  assert not any(
      p.get("property name") == "prop_metric" for p in edge_props_before
  )
  ...
  edge_props_after = schema_after["Edge properties per edge type"][
      edge_table_key
  ]
  assert any(
      p.get("property name") == "prop_distance" for p in edge_props_after
  )
...
def test_flexible_schema_static_node_and_edge_properties_handling(
  ...
  try:
    ...
    assert any(
        p["property name"]
        == ElementSchema.PROPERTY_PREFIX + "reliability_score"
        for p in edge_props_schema
    )
...
def test_schema_with_empty_properties_for_nodes_and_edges(
  ...
  for graph_store in [
    ...
    node1 = EntityNode(
        label="TEST_EMPTY", name="NodeWithNoProps", properties={}
    )
    node2 = EntityNode(
        label="TEST_EMPTY",
        name="NodeWithProps",
...
def test_get_schema_str_for_dynamic_store(
  ...
  assert (
      f"{graph_store.schema.graph_name}_NODE" in schema_str
  )  # e.g., graphName_NODE
  assert (
      f"{graph_store.schema.graph_name}_EDGE" in schema_str
  )  # e.g., graphName_EDGE
...
def test_get_nodes_by_multiple_properties(
  ...
  # Your SpannerPropertyGraphStore.get() uses update_condition, which currently
  # appends conditions with AND. This test verifies that behavior.
  for graph_store in [
      property_graph_store_static,
      property_graph_store_dynamic,
  ]:
...
def test_get_triplets_with_multiple_filters(
  ...
  for graph_store in [
    ...
    # Setup nodes
    alice = EntityNode(
        label="PERSON", name="Alice", properties={"country": "UK"}
    )
    ...
    triplets2 = graph_store.get_triplets(
        ids=[alice.id], relation_names=["MANAGES"]
    )
...
def test_delete_nodes_with_multiple_criteria(
  ...
  for graph_store in [
    ...
    assert (
        "Ruler" not in remaining_names
    ), "Node4 (Ruler) should have been deleted"
    assert {
        "Book",
        "Pen",
        "Eraser",
    ...
    graph_store.delete(
        entity_names=["Book"], properties={"category": "Fiction"}
    )
    ...
    assert not graph_store.get(
        ids=[node1.id]
    )  # Book was already deleted in Case 2
    # Let's check Pen
    assert (
        len(graph_store.get(ids=[node2.id])) == 1
    )  # Pen should still be there.
...
def test_delete_relations_by_label_in_non_flexible_schema_impact(
  ...
  uses_edge_table_key_before = (
      f"{graph_store.schema.graph_name}_USER_S_USES_S_APP_S"
  )
  ...
  likes_triplets_after_delete = graph_store.get_triplets(
      relation_names=["LIKES_S"]
  )
...
def test_get_triplets_filter_by_relation_properties(
  ...
  for graph_store in [
    ...
    user_a = EntityNode(
        label="U", name="UserA", properties={"status": "active"}
    )
    user_b = EntityNode(
...
def test_vector_query_with_exact_match_filter(
  ...
  for graph_store in [
    ...
    # schema/indexing
    graph_store.clean_up()  # Or ensure fixtures provide truly isolated instances
...
def test_vector_query_top_k_variations(
  ...
  for graph_store in [
    ...
    query_k0 = VectorStoreQuery(
        query_embedding=query_embedding, similarity_top_k=0
    )
    ...
    # Case 2: top_k = 1
    query_k1 = VectorStoreQuery(
        query_embedding=query_embedding, similarity_top_k=1
    )
...
def test_get_rel_map_depth_variations_and_limit(
  ...
  for graph_store in [
    ...
    triplets_d1 = graph_store.get_rel_map(
        graph_nodes=[node_a], depth=1, limit=10
    )
    ...
    triplets_d2 = graph_store.get_rel_map(
        graph_nodes=[node_a], depth=2, limit=10
    )
    ...
    triplets_d4 = graph_store.get_rel_map(
        graph_nodes=[node_a], depth=4, limit=10
    )
...
def test_get_rel_map_on_disconnected_nodes(
  ...
  for graph_store in [
    ...
    triplets_target = graph_store.get_rel_map(
        graph_nodes=[node_target], depth=1
    )
...
def test_get_rel_map_with_cycles(
  ...
  for graph_store in [
    ...
    rel_ab = Relation(
        source_id=node_a_cyc.id, target_id=node_b_cyc.id, label="AB"
    )
    rel_bc = Relation(
        source_id=node_b_cyc.id, target_id=node_c_cyc.id, label="BC"
    )
    rel_ca = Relation(
