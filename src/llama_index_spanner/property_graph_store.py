...
def node_from_json(
    label: str, json_node_properties: Dict[str, Any]
) -> LabelledNode:
...
USER_AGENT_GRAPH_STORE = (
    "langchain-google-spanner-python:graphstore/" + __version__
)
...
class SpannerPropertyGraphStore(PropertyGraphStore):
    ...
    for r in relations:
      if r.source_id not in node_mapping or r.target_id not in node_mapping:
        if self.ignore_invalid_relations:
          ...
          raise ValueError("Some relations have unknown source or target ids.")
  ...
  def get(
    ...
    for record in data:
      nodes.append(
          node_from_json(record["type"], record["json_node_properties"])
      )
    return nodes
